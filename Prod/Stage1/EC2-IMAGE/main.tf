...
resource "null_resource" "run_imagebuilder_once_web" {
  triggers = {
    pipeline_arn = aws_imagebuilder_image_pipeline.web_ami_pipeline.arn
  }

  provisioner "local-exec" {
    command = <<-EOT
      aws imagebuilder start-image-pipeline-execution \
        --image-pipeline-arn ${aws_imagebuilder_image_pipeline.web_ami_pipeline.arn}
    EOT
  }
}
